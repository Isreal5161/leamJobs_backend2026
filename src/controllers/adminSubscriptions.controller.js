...
export const trialSettings = async (req, res, next) => { try { return res.status(200).json({ success: true, data: { settings: await getAdminSubscriptionTrialSettings() } }); } catch (error) { return next(error); } };
export const updateTrialSettings = async (req, res, next) => { try { return res.status(200).json({ success: true, data: { settings: await updateAdminSubscriptionTrialSettings(req.validatedTrialSettings) } }); } catch (error) { return next(error); } };
